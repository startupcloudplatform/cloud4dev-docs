...
- 하나의 애플리케이션 내에 모든 로직들이 모두 들어있는 “단일구조” : 도메인 로직은 Class, Function, Package 등으로 구분 
- 모든 요청은 하나의 프로세스에서 처리 
- 개발이 완료되면 전체 로직들에 대한 테스트가 진행되고 전체 프로그램이 빌드되어서 서버에 배포 
- 각 컴포넌트들의 상호 호출은 함수를 이용한 Call-by-Reference 구조 → 성능 제약이 덜함 
- 물리적인 서버 또는 가상화 서버에 동일한 인스턴스 전체가 배포되는 것으로 수평 확장됨
...
- 컴포넌트들이 서로 로컬 콜 기반으로 단단하게 결합되어 있어 특정 컴포넌트나 모듈에서의 성능 문제나 장애가 다른 컴포넌트에 까지 영향을 끼침
...
3. **데이터 분리**
   ...
   - 서비스 별로 필요에 따라 별도의 데이터베이스를 사용
   - 서비스가 API에서부터 데이터베이스까지 분리되는 수직 분할 원칙 (Vertical Slicing)에 따름
   - 데이터베이스의 종류 자체를 다르게 하거나, 같은 데이터베이스를 사용하더라도 스키마를 나누는 방법 사용 
4. **API Gateway**
